...
class VariablePropertyView : public Dumpable {
public:
  using Property = VariableProperty;
  using Traits = modeling::matching::VariableTraits<VariableProperty>;
...
public:
  explicit VariablePropertyView(std::shared_ptr<VariableProperty> property)
      : VariablePropertyView(getId(*property), property,
                             getIndices(*property)) {}

private:
  VariablePropertyView(Id id, std::shared_ptr<VariableProperty> property,
                       IndexSet visibleIndices)
      : id(std::move(id)), property(std::move(property)),
        visibleIndices(std::move(visibleIndices)) {}
...
public:
  ...
  const IndexSet &getIndices() const { return visibleIndices; }

  void hideIndices(const IndexSet &indices) {
    assert(getIndices(getProperty()).contains(indices));
    visibleIndices -= indices;
  }

private:
  static Id getId(const VariableProperty &p) { return Traits::getId(&p); }
...
/*
template <typename VariableProperty>
struct VariableTraits<VariableProperty> {
  using Variable = VariableProperty;

  using MatchingTraits = matching::VariableTraits<VariableProperty>;
  using Id = typename MatchingTraits::Id;

  static Id getId(const Variable *variable) {
    return MatchingTraits::getId(&variable->getProperty());
  }

  static size_t getRank(const Variable *variable) {
    return MatchingTraits::getRank(&variable->getProperty());
  }

  static const IndexSet &getIndices(const Variable *variable) {
    return variable->getIndices();
  }

  static llvm::raw_ostream &dump(const Variable *variable,
                                 llvm::raw_ostream &os) {
    return MatchingTraits::dump(&variable->getProperty(), os);
  }
};
*/
...
/*
template <typename EquationProperty>
struct EquationTraits<internal::pantelides::singularity::EquationWrapper<EquationProperty>> {
  using Equation = internal::pantelides::singularity::EquationWrapper<EquationProperty>;

  using MatchingTraits = matching::EquationTraits<EquationProperty>;
  using Id = typename MatchingTraits::Id;

  static Id getId(const Equation *equation) {
    return MatchingTraits::getId(&equation->getProperty());
  }

  static size_t getNumOfIterationVars(const Equation *equation) {
    return MatchingTraits::getNumOfIterationVars(&equation->getProperty());
  }

  static const IndexSet &getIndices(const Equation *equation) {
    return MatchingTraits::getIndices(&equation->getProperty());
  }

  using VariableType = typename MatchingTraits::VariableType;
  using VariableId = typename VariableTraits<VariableType>::Id;

  static std::vector<Access<VariableId>> getAccesses(const Equation *equation) {
    return MatchingTraits::getAccesses(&equation->getProperty());
  }

  static llvm::raw_ostream &dump(const Equation *equation,
                                 llvm::raw_ostream &os) {
    return MatchingTraits::dump(&equation->getProperty(), os);
  }
};
*/
} // namespace matching
  ...
  bool run() {
    ...
    while (!equationsWorkList.empty()) {
      ...
      do {
        ...
        // the visited nodes.
        Frontier frontier;
        frontier.emplace_back(this->getBaseGraph(), equationDescriptor, unmatchedIndices);
...
private:
  ...
  /*
  void addDerivativesMatches(Visits &visits) {
    llvm::DenseMap<
        VertexDescriptor,
        llvm::DenseMap<VertexDescriptor, std::pair<IndexSet, IndexSet>>>
        requestedMatches;

    for (const auto &visitedVariable : visits.getVisitedVariables()) {
      VertexDescriptor variableDescriptor = visitedVariable.first;

      const VariableSubset &variableDerivative =
          getVariableDerivative(variableDescriptor);

      VertexDescriptor variableDerivativeDescriptor =
          variableDerivative.descriptor;

      for (EdgeDescriptor edgeDescriptor :
           llvm::make_range(this->edgesBegin(variableDescriptor),
                            this->edgesEnd(variableDescriptor))) {
        const Edge &edge = this->getBaseGraph()[edgeDescriptor];

        VertexDescriptor equationDescriptor =
            edgeDescriptor.from == variableDescriptor ? edgeDescriptor.to
                                                      : edgeDescriptor.from;

        IndexSet matchedVariableIndices = edge.getMatched().flattenRows();

        if (matchedVariableIndices.empty()) {
          continue;
        }

        IndexSet matchedEquationIndices = edge.getMatched().flattenColumns();

        if (matchedEquationIndices.empty()) {
          continue;
        }

        const EquationSubset &equationDerivative =
            getEquationDerivative(equationDescriptor);

        requestedMatches[variableDerivativeDescriptor]
                        [equationDerivative.descriptor] = {
    ...
    for (auto &match : requestedMatches) {
      VertexDescriptor variableDerivativeDescriptor = match.first;

      for (EdgeDescriptor edgeDescriptor :
           llvm::make_range(this->edgesBegin(variableDerivativeDescriptor),
                            this->edgesEnd(variableDerivativeDescriptor))) {
        Edge &edge = this->getBaseGraph()[edgeDescriptor];

        VertexDescriptor equationDescriptor =
            edgeDescriptor.from == variableDerivativeDescriptor
                ? edgeDescriptor.to
                : edgeDescriptor.from;

        if (!match.second.contains(equationDescriptor)) {
          continue;
        }

        auto &matchIndices = match.second[equationDescriptor];

        MCIM newMatch = edge.getIncidenceMatrix()
                            .filterRows(matchIndices.second)
                            .filterColumns(matchIndices.first);

        edge.addMatch(newMatch);
      }
    }
  }
  */
