...
#define PARSE_OPTION_WITH_MARSHALLING(                                         \
  ...
  if ((VISIBILITY) & options::MC1Option) {                                     \
...
static void parseCodegenArgs(marco::frontend::CodegenOptions &options,
  ...
  if (const llvm::opt::Arg *arg = args.getLastArg(options::OPT_scc_substitution_max_iterations)) {
    llvm::StringRef value = arg->getValue();
    llvm::APSInt numericValue(value);
    options.sccSolvingBySubstitutionMaxIterations = numericValue.getSExtValue();
  }

  if (const llvm::opt::Arg *arg = args.getLastArg(options::OPT_scc_substitution_max_equations)) {
    llvm::StringRef value = arg->getValue();
    llvm::APSInt numericValue(value);
    options.sccSolvingBySubstitutionMaxEquationsInSCC = numericValue.getSExtValue();
  }
