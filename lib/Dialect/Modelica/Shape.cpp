...
#include "marco/Dialect/Modelica/Shape.h"
...
namespace marco
{
  ...
  long Shape::DimensionSize::getNumericValue() const
  {
    ...
    if (isUndefined()) return -1;//todo: remove and add assert (?)
  ...
  std::vector<std::vector<long>> listIndexes(std::vector<long> partialIndex, llvm::ArrayRef<Shape::DimensionSize> nextDimensions, size_t ragged_depth = 0)
  {
    ...
    for (long it = 0; it < getCurrentRaggedDimension(partialIndex, dim, ragged_depth); ++it) {
      auto p = partialIndex;
      p.push_back(it);
  ...
  llvm::hash_code hash_value(const Shape& S)
  {
    return llvm::hash_combine_range(S.begin(), S.end());
  }
