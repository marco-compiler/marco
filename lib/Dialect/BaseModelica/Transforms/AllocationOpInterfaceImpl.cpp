...
struct ArrayBroadcastOpInterface
    : public AllocationOpInterface::ExternalModel<ArrayBroadcastOpInterface,
                                                  ArrayBroadcastOp> {
  static std::optional<mlir::Operation *> buildDealloc(mlir::OpBuilder &builder,
                                                       mlir::Value alloc) {
    return builder.create<FreeOp>(alloc.getLoc(), alloc).getOperation();
...
struct RawVariableOpInterface
    : public AllocationOpInterface::ExternalModel<RawVariableOpInterface,
                                                  RawVariableOp> {
  static std::optional<mlir::Operation *> buildDealloc(mlir::OpBuilder &builder,
                                                       mlir::Value alloc) {
    auto deallocOp =
        builder.create<RawVariableDeallocOp>(alloc.getLoc(), alloc);
  ...
  buildPromotedAlloc(mlir::OpBuilder &builder, mlir::Value alloc) {
    ...
    auto stackRawVariableOp = builder.create<RawVariableOp>(
        rawVariableOp.getLoc(), rawVariableOp.getResult().getType(),
        rawVariableOp.getName(), rawVariableOp.getDimensionsConstraints(),
        rawVariableOp.getDynamicSizes(), rawVariableOp.getOutput(), false);
