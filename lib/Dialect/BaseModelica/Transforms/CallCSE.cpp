  ...
  for (auto callOpWithInductionRanges : callOpsWithInductionRanges) {
    ...
    for (CallEquivalenceGroup &callEquivalenceGroup : callEquivalenceGroups) {
      ...
      llvm::SmallVector<std::pair<mlir::BlockArgument, mlir::BlockArgument>>
          inductionAccesses;
      if (!callExpression.isEquivalent(representative.callOp, inductionAccesses,
                                       symbolTableCollection)) {
        continue;
      }

      // If no inductions were accessed the calls are equivalent.
      if (inductionAccesses.empty()) {
      ...
      bool inductionRangesMatch =
          !llvm::any_of(inductionAccesses, [&](auto accessPair) {
      ...
      for (mlir::BlockArgument representativeInductionAccess :
        ...
        for (auto accessPair : inductionAccesses) {
    ...
    if (!foundGroup) {
      ...
      llvm::dyn_cast<EquationExpressionOpInterface>(
          *callOpWithInductionRanges.first)
          .isEquivalent(callOpWithInductionRanges.first, inductionAccesses,
                        symbolTableCollection);
      ...
      CallEquivalenceGroupEntry representativeEntry = {
          ...
          .templateInductionRanges = callOpWithInductionRanges.second,
          .orderedUsedInductions =
              llvm::map_to_vector(inductionAccesses, [](auto &accessPair) {
...
/// Decrement the value of [inductions] by one.
/// Returning the resultant values.
llvm::SmallVector<mlir::Value> shiftInductionsDown(mlir::ValueRange inductions,
                                                   mlir::RewriterBase &rewriter,
                                                   mlir::Location loc) {
  auto offset = rewriter.create<ConstantOp>(loc, rewriter.getIndexAttr(-1));
  llvm::SmallVector<mlir::Value> shiftedIndices;
  for (mlir::Value index : inductions) {
    shiftedIndices.push_back(rewriter.create<AddOp>(loc, index, offset));
  }
...
emitDriverEquationTemplate(const CallEquivalenceGroupEntry &representative,
  ...
  if (numInductions == 0) {
    ...
    // Extract cse tensor slot with template inductions.
    // The cse tensor is 0-based, while the inductions are 1-based, so they must
    // be shifted down by one.
    llvm::SmallVector<mlir::Value> shiftedInductionVariables =
        shiftInductionsDown(cseEquationTemplateOp.getInductionVariables(),
                            rewriter, loc);
  ...
  for (const CallEquivalenceGroupEntry &call : calls) {
    ...
    llvm::SmallVector<mlir::Value> results =
        llvm::map_to_vector(cseVariables, [&](VariableOp cseVariable) {
          ...
          // Induction ranges are 1-based while tensors are 0-based, so the
          // inductions must be shifted down by one.
          ...
          auto shiftedInductions =
              shiftInductionsDown(inductionVariables, rewriter, loc);
          return rewriter
