...
public:
  ...
  struct Node {
    ...
    Node(const CallGraph *graph, mlir::Operation *op) : graph(graph), op(op) {}
  ...
  /// entry node.
  size_t getNumOfNodes() const { return nodes.size(); }

  /// Get the entry node of the graph.
  const Node &getEntryNode() const { return nodes[0]; }
...
struct DenseMapInfo<::CallGraph::Node> {
  ...
  static bool isEqual(const ::CallGraph::Node &lhs,
...
struct GraphTraits<const ::CallGraph *> {
  ...
  using ChildIteratorType = std::set<::CallGraph::Node>::const_iterator;
  ...
  using ChildEdgeIteratorType = std::set<::CallGraph::Node>::const_iterator;
...
static bool canBeInlined(FunctionOp functionOp) {
  ...
  while (!nestedOps.empty()) {
    ...
    if (mlir::isa<VariableSetOp, VariableComponentSetOp>(nestedOp)) {
      return false;
    }
...
protected:
  virtual std::set<llvm::StringRef> getDependencies(VariableOp variable) = 0;
...
struct DenseMapInfo<::VariablesDependencyGraph::Node> {
  ...
  static bool isEqual(const ::VariablesDependencyGraph::Node &lhs,
...
namespace {
[[maybe_unused]] bool VariablesDependencyGraph::hasCycles() const {
  auto beginIt = llvm::scc_begin(this);
  auto endIt = llvm::scc_end(this);
...
DefaultValuesGraph::getDependencies(VariableOp variable) {
...
private:
  void collectGraphNodes(CallGraph &callGraph,
