...
private:
  ...
  mlir::LogicalResult createRangedStateVariableUpdateBlocks(
      mlir::OpBuilder &builder,
      mlir::SymbolTableCollection &symbolTableCollection,
      mlir::ModuleOp moduleOp, DynamicOp dynamicOp, VariableOp stateVariable,
      VariableOp derivativeVariable, GlobalVariableOp timeStepVariable);
...
mlir::LogicalResult EulerForwardPass::solveMainModel(
    ...
    llvm::ArrayRef<VariableOp> variables, llvm::ArrayRef<SCCOp> SCCs) {
  auto moduleOp = modelOp->getParentOfType<mlir::ModuleOp>();

  if (mlir::failed(createUpdateNonStateVariablesFunction(rewriter, moduleOp,
                                                         modelOp, SCCs))) {
    return mlir::failure();
  }
...
mlir::LogicalResult EulerForwardPass::createUpdateStateVariablesFunction(
  ...
  auto timeStepArray = builder.create<GlobalVariableGetOp>(timeStepArg.getLoc(),
                                                           timeStepVariable);
  ...
  if (!stateVariables.empty()) {
    ...
    auto scheduleOp = builder.create<ScheduleOp>(modelOp.getLoc(),
                                                 "schedule_state_variables");
...
static void getStateUpdateBlockVarWriteReadInfo(
    mlir::OpBuilder &builder, VariableOp stateVariable,
    VariableOp derivativeVariable, MultidimensionalRangeAttr ranges,
...
static void createStateUpdateFunctionCall(mlir::OpBuilder &builder,
  ...
  auto blockOp = builder.create<ScheduleBlockOp>(stateVariable.getLoc(), true);

  getStateUpdateBlockVarWriteReadInfo(builder, stateVariable,
                                      derivativeVariable, ranges,
...
mlir::LogicalResult EulerForwardPass::createRangedStateVariableUpdateBlocks(
    mlir::OpBuilder &builder,
    mlir::SymbolTableCollection &symbolTableCollection, mlir::ModuleOp moduleOp,
    DynamicOp dynamicOp, VariableOp stateVariable,
    VariableOp derivativeVariable, GlobalVariableOp timeStepVariable) {
  mlir::OpBuilder::InsertionGuard guard(builder);
  ...
  auto getNewScalarStateFn = [&](mlir::OpBuilder &nestedBuilder,
    ...
    mlir::Value result = nestedBuilder.create<MulOp>(
        nestedLoc, scalarDerivative.getType(), scalarDerivative, timeStep);

    result = nestedBuilder.create<AddOp>(nestedLoc, scalarState.getType(),
                                         scalarState, result);
  ...
  if (variableRank == 0) {
    ...
    builder.create<QualifiedVariableSetOp>(equationFuncOp.getLoc(),
                                           stateVariable, updatedState);
  } else {
    // Array variable.
    mlir::Value state = builder.create<QualifiedVariableGetOp>(
    ...
    mlir::affine::buildAffineLoopNest(
        ...
        [&](mlir::OpBuilder &nestedBuilder, mlir::Location nestedLoc,
          ...
          mlir::Value scalarState =
              nestedBuilder.create<LoadOp>(nestedLoc, state, indices);

          mlir::Value scalarDerivative =
              nestedBuilder.create<LoadOp>(nestedLoc, derivative, indices);

          mlir::Value updatedScalarState = getNewScalarStateFn(
              nestedBuilder, nestedLoc, scalarState, scalarDerivative);

          nestedBuilder.create<StoreOp>(nestedLoc, updatedScalarState, state,
                                        indices);
        });
  }
...
mlir::LogicalResult EulerForwardPass::createMonolithicStateVariableUpdateBlock(
  ...
  mlir::Value mulOp =
      builder.create<MulOp>(funcOp.getLoc(), timeStep, derivative);

  mlir::Value addOp = builder.create<AddOp>(funcOp.getLoc(), state, mulOp);

  builder.create<QualifiedVariableSetOp>(funcOp.getLoc(), stateVariable, addOp);
