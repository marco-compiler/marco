...
void TensorExtractOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                                  mlir::Location loc) {
...
namespace mlir::bmodelica {
void TensorInsertOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                                 mlir::Location loc) {
  mlir::Value operand = getDestination();
  mlir::ValueRange indices = getIndices();
...
namespace mlir::bmodelica {
void TensorInsertSliceOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                                      mlir::Location loc) {
...
void DimOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
...
void LoadOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                         mlir::Location loc) {
  mlir::ValueRange indices = getIndices();
  ...
  if (indices.size() > 0) {
    ...
    operand =
        builder.create<ArrayToTensorOp>(loc, operandTensorType, getArray());
...
void StoreOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                          mlir::Location loc) {
  mlir::ValueRange indices = getIndices();
...
void SubscriptionOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                                 mlir::Location loc) {
  // Modelica arrays will eventually be converted to MLIR tensors
  // at some point down the pipeline, so convert everything
  // to tensor to avoid issues
  ...
  auto operandTensorType = mlir::RankedTensorType::get(
      operandShapedType.getShape(), operandShapedType.getElementType());
  auto tensorOperand =
      builder.create<ArrayToTensorOp>(loc, operandTensorType, getSource());
...
void DivOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
  mlir::Value rhs = getRhs();
...
void DivEWOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                          mlir::Location loc) {
  mlir::Value rhs = getRhs();
...
void AcosOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                         mlir::Location loc) {
  mlir::Value operand = getOperand();
...
void AsinOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                         mlir::Location loc) {
  mlir::Value operand = getOperand();
...
void DivTruncOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                             mlir::Location loc) {
  mlir::Value rhs = getY();
...
void LogOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
  mlir::Value operand = getOperand();
...
void Log10Op::generateRuntimeVerification(mlir::OpBuilder &builder,
                                          mlir::Location loc) {
  mlir::Value operand = getOperand();
...
void ModOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
  mlir::Value rhs = getY();
...
void RemOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
  mlir::Value rhs = getY();
...
void SizeOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                         mlir::Location loc) {
  if (getNumOperands() == 2) {
    mlir::Value dim = getDimension();
    auto arrayShapedType = mlir::cast<mlir::ShapedType>(getArray().getType());
...
void SqrtOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                         mlir::Location loc) {
  mlir::Value operand = getOperand();
  verifyArgumentIsPositive(
...
void TanOp::generateRuntimeVerification(mlir::OpBuilder &builder,
                                        mlir::Location loc) {
  mlir::Value operand = getOperand();
...
namespace mlir::bmodelica {
mlir::ParseResult EquationInstanceOp::parse(mlir::OpAsmParser &parser,
                                            mlir::OperationState &result) {
  mlir::OpAsmParser::UnresolvedOperand templateOperand;
  ...
  while (mlir::succeeded(parser.parseOptionalComma())) {
    if (mlir::succeeded(parser.parseOptionalKeyword("indices"))) {
    ...
    } else if (mlir::succeeded(parser.parseOptionalKeyword("match"))) {
      // Matched variable.
      Variable &prop = result.getOrAddProperties<Properties>().match;

      if (parser.parseEqual() ||
          mlir::failed(::mlir::bmodelica::parse(parser, prop))) {
        return mlir::failure();
      }
    } else if (mlir::succeeded(parser.parseOptionalKeyword("schedule"))) {
      // Schedule.
      ScheduleList &prop = result.getOrAddProperties<Properties>().schedule;
...
void EquationInstanceOp::print(mlir::OpAsmPrinter &printer) {
  printer << " " << getTemplate().getResult();
  ...
  if (const Variable &match = getProperties().match) {
    printer << ", match = ";
    ::mlir::bmodelica::print(printer, match);
  }

  if (const ScheduleList &schedule = getProperties().schedule;
      !schedule.empty()) {
    printer << ", schedule = ";
    ::mlir::bmodelica::print(printer, schedule);
  }
...
mlir::LogicalResult EquationInstanceOp::verify() {
  // Check the indices for the inductions.
  size_t numOfInductions = getInductionVariables().size();

  if (size_t indicesRank = getProperties().indices.rank();
  ...
  if (size_t numOfSchedules = getProperties().schedule.size();
      numOfSchedules != 0 && numOfSchedules != numOfInductions) {
    return emitOpError()
           << "Unexpected number of iteration directions (expected "
           << numOfInductions << ", got " << numOfSchedules << ")";
  }
...
mlir::LogicalResult EquationInstanceOp::setIndices(
  ...
  // This must be done after fetching the writes, as the discovery relies on the
  // indices to filter the accesses.
  getProperties().indices = std::move(indices);
...
mlir::LogicalResult EquationInstanceOp::getWriteAccesses(
    llvm::SmallVectorImpl<VariableAccess> &result,
    mlir::SymbolTableCollection &symbolTableCollection,
    llvm::ArrayRef<VariableAccess> accesses) {
  return getWriteAccesses(result, symbolTableCollection,
                          getProperties().indices, accesses);
}
...
mlir::LogicalResult EquationInstanceOp::getReadAccesses(
    llvm::SmallVectorImpl<VariableAccess> &result,
    mlir::SymbolTableCollection &symbolTableCollection,
    llvm::ArrayRef<VariableAccess> accesses) {
  return getReadAccesses(result, symbolTableCollection, getProperties().indices,
                         accesses);
}
...
mlir::LogicalResult EquationInstanceOp::explicitate(
    mlir::RewriterBase &rewriter,
    mlir::SymbolTableCollection &symbolTableCollection) {
...
EquationInstanceOp EquationInstanceOp::cloneAndExplicitate(
    mlir::RewriterBase &rewriter,
    mlir::SymbolTableCollection &symbolTableCollection) {
  ...
  auto result = rewriter.create<EquationInstanceOp>(getLoc(), clonedTemplate);
...
mlir::LogicalResult EquationInstanceOp::cloneWithReplacedAccess(
  ...
  for (auto &[assignedIndices, equationTemplateOp] : templateResults) {
    ...
    if (mlir::failed(clonedOp.setIndices(slicedAssignedIndices,
