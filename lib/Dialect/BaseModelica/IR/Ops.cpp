...
static mlir::LogicalResult cleanEquationTemplates(
      ...
      : public mlir::OpRewritePattern<TensorViewOp>
  {
        ...
        TensorViewOp op, mlir::PatternRewriter& rewriter) const override
    {
      ...
      if (inferredResultType != op.getResult().getType()) {
        auto newOp = rewriter.create<TensorViewOp>(
            op.getLoc(), inferredResultType, op.getSource(),
            op.getSubscriptions());
...
namespace mlir::bmodelica
{
  void TensorViewOp::build(
      mlir::OpBuilder& builder,
      mlir::OperationState& state,
      mlir::Value source,
      mlir::ValueRange subscriptions)
  {
    mlir::TensorType resultType = inferResultType(
        source.getType().cast<mlir::TensorType>(), subscriptions);
  ...
  {
    patterns.add<
        InferTensorViewResultTypePattern,
        MergeTensorViewsPattern>(context);
  }

  mlir::TensorType TensorViewOp::inferResultType(
      mlir::TensorType source, mlir::ValueRange indices)
  {
...
namespace mlir::bmodelica
{
  mlir::ParseResult TensorExtractOp::parse(
...
namespace mlir::bmodelica
{
      ...
      mlir::OpBuilder& builder, mlir::Location loc)
  {
    ...
    mlir::Value operand = builder.create<CastOp>(
...
namespace mlir::bmodelica
{
      ...
      mlir::OpAsmParser& parser, mlir::OperationState& result)
  {
    ...
    if (parser.parseOperand(array) ||
        ...
        parser.parseColonType(arrayType) ||
        parser.resolveOperand(value, arrayType.cast<mlir::ShapedType>().getElementType(), result.operands) ||
        parser.resolveOperand(array, arrayType, result.operands)) {
      return mlir::failure();
    }
...
namespace mlir::bmodelica
{
  ...
  mlir::ParseResult VariableSetOp::parse(
      mlir::OpAsmParser& parser, mlir::OperationState& result)
  {
    auto loc = parser.getCurrentLocation();
    ...
    if (variableAttr) {
      result.getOrAddProperties<VariableSetOp::Properties>().variable = variableAttr;
    }
    ...
    if (!indices.empty()) {
      if (parser.resolveOperands(
              indices, llvm::ArrayRef(types).drop_back(), loc,
              result.operands)) {
        return mlir::failure();
      }
    }
...
namespace mlir::bmodelica
{
      ...
      mlir::SymbolTableCollection& symbolTableCollection)
  {
    ...
    mlir::Type expectedResultType =
        componentVariable.getVariableType().unwrap();

    if (!expectedResultShape.empty()) {
      if (auto expectedResultShapedType =
              mlir::dyn_cast<mlir::ShapedType>(expectedResultType)) {
        expectedResultType =
            expectedResultShapedType.clone(expectedResultShape);
      } else {
        expectedResultType = mlir::RankedTensorType::get(
            expectedResultShape, expectedResultType);
      }
    }
...
namespace mlir::bmodelica
{
  ...
  mlir::ParseResult QualifiedVariableSetOp::parse(
      mlir::OpAsmParser& parser, mlir::OperationState& result)
  {
    auto loc = parser.getCurrentLocation();
    ...
    if (variableAttr) {
      result.getOrAddProperties<
          QualifiedVariableSetOp::Properties>().variable = variableAttr;
    }
...
namespace mlir::bmodelica
{
      ...
      llvm::SmallVectorImpl<mlir::Type>& returnTypes)
  {
    ...
    if (isScalar(lhsType) && rhsShapedType) {
      ...
      returnTypes.push_back(mlir::RankedTensorType::get(
          rhsShapedType.getShape(), resultElementType));
    ...
    if (lhsShapedType && isScalar(rhsType)) {
      ...
      returnTypes.push_back(mlir::RankedTensorType::get(
          lhsShapedType.getShape(), resultElementType));
...
namespace mlir::bmodelica
{
      ...
      mlir::OpBuilder& builder, mlir::Location loc)
  {
    ...
    // convert operand to arith-compatible type
    mlir::Value operandCast = builder.create<CastOp>(
      ...
      llvm::SmallVectorImpl<mlir::Type>& returnTypes)
  {
    ...
    if (baseShapedType && exponentShapedType) {
      ...
      mlir::Type resultElementType = inferResultType(
          baseShapedType.getElementType(),
          exponentShapedType.getElementType());
    ...
    for (auto [trueValue, falseValue] : llvm::zip(
      ...
      if (trueValueType == falseValueType) {
        ...
        auto trueValueShapedType =
            trueValueType.dyn_cast<mlir::ShapedType>();

        auto falseValueShapedType =
            falseValueType.dyn_cast<mlir::ShapedType>();
        ...
        if (trueValueShapedType && falseValueShapedType) {
          ...
          mlir::Type resultElementType = getMostGenericScalarType(
              trueValueShapedType.getElementType(),
...
namespace mlir::bmodelica
{
      ...
      mlir::OpBuilder& builder, mlir::Location loc)
  {
    ...
    builder.create<AssertOp>(
        loc, operandCast, builder.getStringAttr(
          "Model error: division by zero"));
  }
}
...
namespace mlir::bmodelica
{
      ...
      llvm::SmallVectorImpl<mlir::Type>& returnTypes)
  {
    ...
    returnTypes.push_back(
        mlir::cast<mlir::Type>(matrixShapedType.clone(shape)));

    return mlir::success();
  }

  bool TransposeOp::isCompatibleReturnTypes(
      mlir::TypeRange lhs, mlir::TypeRange rhs)
  {
    if (lhs.size() != rhs.size()) {
      return false;
...
namespace mlir::bmodelica
{
      ...
      llvm::SmallVectorImpl<std::pair<IndexSet, EquationTemplateOp>>& results)
  {
    ...
    if (destinationRank < sourceRank) {
      ...
      while (mlir::isa<TensorExtractOp, TensorViewOp>(replacedValueOp) &&
        ...
        if (auto extractOp =
                mlir::dyn_cast<TensorExtractOp>(replacedValueOp)) {
        ...
        if (auto viewOp = mlir::dyn_cast<TensorViewOp>(replacedValueOp)) {
          ...
          for (size_t i = 0; i < numOfSubscripts &&
               !allAdditionalIndicesCollected(); ++i) {
            additionalSubscriptionIndices.push_back(
      ...
      IndexSet& remainingEquationIndices)
  {
    ...
    if (equationIndices) {
      remainingEquationIndices -= equationIndices->get();
      results.emplace_back(IndexSet(equationIndices->get()), newEquationTemplateOp);
    } else {
    ...
    if (mlir::failed(mapInductionVariables(
            ...
            mapping, replacementEquation, newEquationTemplateOp,
            access.getPath(), transformation))) {
      return mlir::failure();
    }
    ...
    mlir::Value replacement =
        mapping.lookup(replacementEquation.getValueAtPath(
...
namespace mlir::bmodelica
{
  ...
  */
  ...
  bool RawVariableOp::isProtected()
  {
    return !getOutput();
  }
