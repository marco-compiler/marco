...
DimensionAccess::Redirect::Redirect(DimensionAccess::Redirect &&other) =
    default;

DimensionAccess::Redirect::~Redirect() = default;

DimensionAccess::Redirect &
DimensionAccess::Redirect::operator=(const DimensionAccess::Redirect &other) {
...
void swap(DimensionAccess::Redirect &first, DimensionAccess::Redirect &second) {
  using std::swap;
...
DimensionAccess::build(mlir::AffineExpr expression) {
  if (auto constantExpr =
          mlir::dyn_cast<mlir::AffineConstantExpr>(expression)) {
    return std::make_unique<DimensionAccessConstant>(constantExpr.getContext(),
                                                     constantExpr.getValue());
  }

  if (auto dimExpr = mlir::dyn_cast<mlir::AffineDimExpr>(expression)) {
    return std::make_unique<DimensionAccessDimension>(dimExpr.getContext(),
                                                      dimExpr.getPosition());
  }
  ...
  if (auto binaryExpr = mlir::dyn_cast<mlir::AffineBinaryOpExpr>(expression)) {
    ...
    if (kind == mlir::AffineExprKind::Add) {
      return std::make_unique<DimensionAccessAdd>(
          binaryExpr.getContext(), DimensionAccess::build(binaryExpr.getLHS()),
...
std::unique_ptr<DimensionAccess>
DimensionAccess::operator+(const DimensionAccess &other) const {
  return std::make_unique<DimensionAccessAdd>(getContext(), this->clone(),
                                              other.clone());
}

std::unique_ptr<DimensionAccess>
DimensionAccess::operator-(const DimensionAccess &other) const {
  return std::make_unique<DimensionAccessSub>(getContext(), this->clone(),
                                              other.clone());
}

std::unique_ptr<DimensionAccess>
DimensionAccess::operator*(const DimensionAccess &other) const {
  return std::make_unique<DimensionAccessMul>(getContext(), this->clone(),
                                              other.clone());
}

std::unique_ptr<DimensionAccess>
DimensionAccess::operator/(const DimensionAccess &other) const {
  return std::make_unique<DimensionAccessDiv>(getContext(), this->clone(),
                                              other.clone());
}
...
bool DimensionAccess::isAffine() const { return false; }
