...
namespace marco::vf {
Parser::Parser(VariableFilter &vf, clang::DiagnosticsEngine &diagnosticsEngine,
               clang::SourceManager &sourceManager,
               std::shared_ptr<SourceFile> source)
    : vf(&vf), diagnosticsEngine(&diagnosticsEngine),
      sourceManager(&sourceManager), lexer(source) {
...
Parser::convertLocation(const SourceRange &location) const {
  ...
  auto fileRef =
      fileManager.getVirtualFileRef(location.begin.file->getFileName(), 0, 0);
...
void Parser::emitEmptyRegexError(const SourceRange &location) {
  ...
  auto diagID = diags.getCustomDiagID(clang::DiagnosticsEngine::Error,
...
bool Parser::parseListElement() {
  ...
  if (lookahead[0].isa<TokenKind::LSquare>()) {
    ...
    Tracker tracker(arrayNode->getVariable().getIdentifier(),
                    arrayNode->getRanges());
...
std::optional<ArrayExpression> Parser::parseArray(VariableExpression variable) {
