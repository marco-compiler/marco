...
static bool validateSupportedControlFlow(Region& region)
{
	...
	region.walk([&success](Operation* operation) {
		...
		if (((size == 1 && !operation->getResults().empty()) || size > 1) && !dyn_cast<RegionBranchOpInterface>(operation))
		{
			operation->emitError("All operations with attached regions need to implement the RegionBranchOpInterface.");
			success = false;
		}
	});
	return success;
...
namespace modelica
{
	...
	/// A straight-forward program analysis which detects loop backedges induced by explicit control flow.
	class Backedges
	{
		public:
		using BlockSetT = SmallPtrSet<Block*, 16>;
		using BackedgeSetT = DenseSet<std::pair<Block*, Block*>>;
		...
		[[nodiscard]] BackedgeSetT::const_iterator begin() const
		{
			return edgeSet.begin();
		}

		/// Returns the end iterator to loop over all backedges.
		[[nodiscard]] BackedgeSetT::const_iterator end() const
		{
			return edgeSet.end();}

		private:
		/// Enters the current block and inserts a backedge into the `edgeSet` if we
		/// have already visited the current block. The inserted edge links the given `predecessor` with the `current` block.
	...
	/// The buffer deallocation transformation which ensures that all allocs in the program have a corresponding de-allocation. As a side-effect, it might also introduce copies that in turn leads to additional allocs and de-allocations.
	class BufferDeallocation: BufferPlacementTransformationBase
	{
		...
		void introduceCopies()
		{
			...
			auto findUnsafeValues = [&](Value source, Block* definingBlock) {
				...
				for (Value value : it->second)
				{
					...
					else if (visitedValues.insert(std::make_tuple(value, definingBlock)).second)
			...
			for (Value value : valuesToFree)
			{
				...
				// Register the value to require a final dealloc. Note that we do not have to assign a block here since we do not want to move the allocation node to another location.
		...
		void introduceBlockArgCopy(BlockArgument blockArg)
		{
			...
			for (auto it = block->pred_begin(), e = block->pred_end(); it != e; ++it)
			{
				...
				Value sourceValue =branchInterface.getSuccessorOperands(it.getSuccessorIndex()).getValue()[blockArg.getArgNumber()];
				...
				auto mutableOperands =branchInterface.getMutableSuccessorOperands(it.getSuccessorIndex());
				
				if (!mutableOperands.hasValue())
					terminator->emitError() << "terminators with immutable successor operands are not supported";
				else
			...
			// Check whether the block argument has implicitly defined predecessors via the RegionBranchOpInterface. This can be the case if the current block argument belongs to the first block in a region and the parent operation implements the RegionBranchOpInterface.
			...
			introduceCopiesForRegionSuccessors(
					regionInterface,
					argRegion->getParentOp()->getRegions(),
					blockArg,
			...
			regionInterface.getSuccessorRegions(llvm::None, successorRegions);
			...
			assert(op != parentOp->getOperands().end() && "parentOp does not contain operand");
		...
		void introduceValueCopyForRegionResult(Value value)
		{
			...
			// Introduce a copy for all region "results" that are returned to the parent operation. This is required since the parent's result value has been considered critical. Therefore, the algorithm assumes that a copy of a previously allocated buffer is returned by the operation (like in the case of a block argument).
			introduceCopiesForRegionSuccessors(regionInterface, operation->getRegions(), value, regionPredicate);
		}

		/// Introduces buffer copies for all terminators in the given regions. The
		/// regionPredicate is applied to every successor region in order to restrict the copies to specific regions.
		template<typename TPredicate>
		void introduceCopiesForRegionSuccessors(RegionBranchOpInterface regionInterface, MutableArrayRef<Region> regions, Value argValue, const TPredicate& regionPredicate)
		{
			for (Region& region : regions)
			{
				// Query the regionInterface to get all successor regions of the
				// current one.
				SmallVector<RegionSuccessor, 2> successorRegions;
				regionInterface.getSuccessorRegions(region.getRegionNumber(), successorRegions);
				
				// Try to find a matching region successor.
				RegionSuccessor* regionSuccessor = llvm::find_if(successorRegions, regionPredicate);
		...
		Value introduceBufferCopy(Value sourceValue, Operation* terminator)
		{
			...
			// copying copied buffers can introduce memory leaks since the invariant of BufferPlacement assumes that a buffer will be only copied once into a temporary buffer. Hence, the construction of copy chains introduces
			// additional allocations that are not tracked automatically by the
			// algorithm.
			if (copiedValues.contains(sourceValue))
				return sourceValue;
			// Create a new alloc at the current location of the terminator.
			auto memRefType = sourceValue.getType().cast<PointerType>();
			OpBuilder builder(terminator);

			// Alloc and CopyOp nodes.
			auto alloc = builder.create<ArrayCopyOp>(terminator->getLoc(), sourceValue, true);
...
class BufferDeallocationPass : public PassWrapper<BufferDeallocationPass, OperationPass<FuncOp>>
{
	...
	void runOnOperation() override {
		...
		if (backedges.size() > 0) {
			getOperation().emitError("Structured control-flow loops are supported only.");
