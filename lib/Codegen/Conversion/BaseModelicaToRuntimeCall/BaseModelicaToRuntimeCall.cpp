...
BaseModelicaToRuntimeCallConversionPass::convertOperations() {
  ...
  target.addLegalDialect<BaseModelicaDialect, mlir::arith::ArithDialect,
  ...
  target.addDynamicallyLegalOp<PowOp>(
      [](PowOp op) { return op.getBase().getType().isa<mlir::TensorType>(); });
...
protected:
  ...
  [[nodiscard]] std::string getMangledType(mlir::Type type) const {
    ...
    if ( auto stringType = type.dyn_cast<mlir::runtime::RuntimeStringType>() ) {
