...
namespace
{
  ...
  struct VariableGetterOpLowering : public IDAOpConversion<VariableGetterOp>
  {
        ...
        mlir::ConversionPatternRewriter& rewriter) const override
    {
      ...
      auto newOp = rewriter.replaceOpWithNewOp<mlir::LLVM::LLVMFuncOp>(
  ...
  struct JacobianFunctionOpLowering
      : public IDAOpConversion<JacobianFunctionOp>
  {
      ...
      : public mlir::impl::IDAToLLVMConversionPassBase<IDAToLLVMConversionPass>
  {
    public:
      ...
      void runOnOperation() override
      {
        ...
        // This must be done first and as an independent step, as other
        // operations within the IDA dialect will need the addresses of such
        // functions when being converted.

        if (mlir::failed(convertFunctionsLikeOps())) {
          mlir::emitError(
              getOperation().getLoc(),
              "Error in converting the IDA function-like operations");
