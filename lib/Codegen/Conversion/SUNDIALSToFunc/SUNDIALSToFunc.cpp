...
struct VariableGetterOpLowering
  ...
  using SUNDIALSOpConversionPattern<
      VariableGetterOp>::SUNDIALSOpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(VariableGetterOp op, OpAdaptor adaptor,
    ...
    for (auto variableIndex : llvm::enumerate(op.getVariableIndices())) {
      ...
      mlir::Value variableIndexPtr = rewriter.create<mlir::LLVM::GEPOp>(
          ...
          rewriter.getI64Type(), variableIndicesPtr,
...
struct VariableSetterOpLowering
  ...
  using SUNDIALSOpConversionPattern<
      VariableSetterOp>::SUNDIALSOpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(VariableSetterOp op, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    mlir::Location loc = op.getLoc();
...
struct AccessFunctionOpLowering
  ...
  using SUNDIALSOpConversionPattern<
      AccessFunctionOp>::SUNDIALSOpConversionPattern;

  mlir::LogicalResult
  matchAndRewrite(AccessFunctionOp op, OpAdaptor adaptor,
                  mlir::ConversionPatternRewriter &rewriter) const override {
    mlir::Location loc = op.getLoc();
    ...
    for (auto equationIndex : llvm::enumerate(op.getEquationIndices())) {
      ...
      mlir::Value equationIndexPtr = rewriter.create<mlir::LLVM::GEPOp>(
          ...
          rewriter.getI64Type(), equationIndicesPtr,
          llvm::ArrayRef<mlir::LLVM::GEPArg>{index});
    ...
    for (unsigned int i = 0; i < numOfResults; ++i) {
      ...
      rewriter.create<mlir::LLVM::StoreOp>(variableIndexPtr.getLoc(),
    ...
    rewriter.replaceOpWithNewOp<mlir::cf::BranchOp>(returnOp, exitBlock,
...
static void
populateSUNDIALSToFuncPatterns(mlir::RewritePatternSet &patterns,
                               mlir::LLVMTypeConverter &typeConverter) {
  patterns.insert<VariableGetterOpLowering, VariableSetterOpLowering,
...
private:
  mlir::LogicalResult convertOperations() {
    ...
    target.addIllegalOp<VariableGetterOp, VariableSetterOp, AccessFunctionOp,
                        ReturnOp>();
