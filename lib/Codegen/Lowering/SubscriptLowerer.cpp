...
{
  SubscriptLowerer::SubscriptLowerer(BridgeInterface* bridge)
      : Lowerer(bridge)
  {
  }
  ...
  std::optional<Results> SubscriptLowerer::lower(const ast::Subscript& subscript)
  {
    ...
    const auto optionalResult = lower(*subscript.getExpression());
    if (!optionalResult) {
      return std::nullopt;
    }
    mlir::Value index = optionalResult.value()[0].get(location);
