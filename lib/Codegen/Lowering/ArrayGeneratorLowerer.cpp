...
ArrayGeneratorLowerer::lower(const ast::ArrayGenerator &array) {
    ...
    if (auto constant = current->dyn_cast<ast::ArrayConstant>()) {
      ...
      for (unsigned i = 0; i < inductions; i++) {
        ...
        if (indRange->getNumOfArguments() == 2) {
          ...
          begin =
              indRange->getArgument(0)->cast<ast::Constant>()->as<int64_t>();
      ...
      if (auto arrayChild =
              forGen->getValue()->dyn_cast<ast::ArrayGenerator>()) {
        nestedArrays.push_back(arrayChild);
      }
    }
  }
...
std::optional<Results>
ArrayGeneratorLowerer::lower(const ast::ArrayConstant &array) {
...
std::optional<Results>
ArrayGeneratorLowerer::lower(const ast::ArrayForGenerator &array) {
  // TODO determine minimum required type
  mlir::Type elementType = RealType::get(builder().getContext());
