...
// As a constructor, this function opens the files if they haven't been opened
// yet. Files are static, so they are shared among all instances of
// DatabaseReader. They are not const, however, because reading from them
// changes their state.
...
std::vector<std::string> DatabaseReader::keyToRow(std::ifstream &file,
                                                  llvm::StringRef key) {
  if (!filesOpened) {
    return {};
  }
...
std::vector<Synset> DatabaseReader::getSynsets(llvm::StringRef word) {
  if (!filesOpened) {
    return {};
  }
...
std::vector<std::string> DatabaseReader::offsetToRow(std::ifstream &file,
                                                     int offset) {
  if (offset < 0 || !filesOpened) {
    return {};
  }
...
std::vector<Synset> DatabaseReader::getHypernyms(const Synset &synset) {
  if (synset == -1 || !filesOpened) {
    return {};
  }
