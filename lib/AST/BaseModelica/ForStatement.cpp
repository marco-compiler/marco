...
ForStatement::ForStatement(SourceRange location)
    : Statement(ASTNodeKind::Statement_For, std::move(location)) {}
...
size_t ForStatement::getNumOfForIndices() const { return forIndices.size(); }
...
size_t ForStatement::getNumOfStatements() const { return statements.size(); }
