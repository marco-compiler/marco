...
Modification::Modification(SourceRange location)
    : ASTNode(ASTNodeKind::Modification, std::move(location)) {}

Modification::Modification(const Modification &other) : ASTNode(other) {
...
bool Modification::hasExpression() const { return expression != nullptr; }
...
ClassModification::ClassModification(SourceRange location)
    : ASTNode(ASTNodeKind::ClassModification, std::move(location)) {
...
ElementModification::ElementModification(SourceRange location)
    : Argument(ASTNodeKind::Argument_ElementModification, std::move(location)),
...
bool ElementModification::hasEachProperty() const { return each; }

void ElementModification::setEachProperty(bool value) { each = value; }

bool ElementModification::hasFinalProperty() const { return final; }

void ElementModification::setFinalProperty(bool value) { final = value; }

llvm::StringRef ElementModification::getName() const { return name; }
...
ElementReplaceable::ElementReplaceable(SourceRange location)
    : Argument(ASTNodeKind::Argument_ElementReplaceable, std::move(location)) {}
...
ElementRedeclaration::ElementRedeclaration(SourceRange location)
    : Argument(ASTNodeKind::Argument_ElementRedeclaration,
               std::move(location)) {}
