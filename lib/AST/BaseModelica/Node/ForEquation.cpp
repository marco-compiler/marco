...
ForEquation::ForEquation(const ForEquation &other) : Equation(other) {
...
size_t ForEquation::getNumOfForIndices() const { return forIndices.size(); }
...
size_t ForEquation::getNumOfEquations() const { return equations.size(); }
...
void ForEquation::setEquations(llvm::ArrayRef<std::unique_ptr<ASTNode>> nodes) {
  equations.clear();
