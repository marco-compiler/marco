...
Annotation::Annotation(const Annotation &other) : ASTNode(other) {
  ...
  for (const auto &argument : getProperties()->getArguments()) {
    if (auto *elementModification = argument->dyn_cast<ElementModification>()) {
      ...
      if (modification->hasClassModification()) {
        for (const auto &derivativeArgument :
             modification->getClassModification()->getArguments()) {
          if (auto *derivativeModification =
                  derivativeArgument->dyn_cast<ElementModification>()) {
            if (derivativeModification->getName() == "order") {
              order = derivativeModification->getModification()
                          ->getExpression()
                          ->dyn_cast<Constant>()
                          ->as<int64_t>();
            }
  ...
  for (const auto &argument : getProperties()->getArguments()) {
    if (auto *elementModification = argument->dyn_cast<ElementModification>()) {
      if (elementModification->getName().equals_insensitive("inverse")) {
        ...
        for (const auto &inverseDeclaration :
             modification->getClassModification()->getArguments()) {
          const auto &inverseDeclarationFullExpression =
              inverseDeclaration->cast<ElementModification>();
          ...
          result.addInverse(
              ...
              call->getCallee()->cast<ComponentReference>()->getName(), args);
        }
      }
    }
  }
