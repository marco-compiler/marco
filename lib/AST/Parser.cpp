...
#include "marco/Utils/ParserUtils.h"
    ...
    while (current != Token::End) {
      ...
      while (current != Token::EndKeyword && current != Token::AnnotationKeyword) {
        if (current == Token::EquationKeyword) {
          ...
          equationsBlocks.push_back(std::move(*eqBlock));
          continue;
        }
        ...
        if (accept(Token::PublicKeyword)) {
          if (auto error = elementList(members, true)) {
        ...
        if (accept(Token::ProtectedKeyword)) {
          if (auto error = elementList(members, false)) {
            return std::move(error);
          }
        ...
        if (firstElementListParsable) {
          if (auto error = elementList(members)) {
            return std::move(error);
          }
        }
      }
  ...
  llvm::Expected<std::unique_ptr<EquationsBlock>> Parser::equationsBlock()
  {
    ...
    while (
        ...
        current != Token::InitialKeyword &&
        current != Token::AlgorithmKeyword &&
        current != Token::ExternalKeyword &&
        current != Token::AnnotationKeyword &&
        current != Token::EndKeyword) {
      if (current == Token::ForKeyword) {
        if (auto error = forEquation(forEquations, 0)) {
          return std::move(error);
        }
      } else {
        TRY(eq, equation());
    ...
    return std::make_unique<EquationsBlock>(location, equations, forEquations);
  }
  ...
  llvm::Error Parser::forEquation(llvm::SmallVectorImpl<std::unique_ptr<ForEquation>>& equations, int nestingLevel)
  {
    ...
    while (!accept<Token::EndKeyword>()) {
      ...
      if (auto error = forEquationBody(inner, nestingLevel + 1)) {
        return error;
      }
  ...
  llvm::Error Parser::forEquationBody(llvm::SmallVectorImpl<std::unique_ptr<ForEquation>>& equations, int nestingLevel)
  {
    ...
    if (auto error = forEquation(equations, nestingLevel)) {
      return error;
    }
