...
bool ExternalRef::hasLanguageSpecification() const {
  return !languageSpecification.empty();
}

bool ExternalRef::hasExternalFunctionCall() const {
  return externalFunctionCall != nullptr;
}

bool ExternalRef::hasAnnotationClause() const {
  return annotationClause != nullptr;
}
} // namespace marco::ast
