...
static llvm::Error resolveDummyReferences(StandardFunction& function)
{
...
static llvm::Error resolveDummyReferences(Model& model)
{
...
namespace marco::ast
{
  ...
  llvm::Error TypeChecker::run<Model>(Class& cls)
  {
    ...
    for (auto& equationsBlock : model->getEquationsBlocks()) {
      for (auto& equation : equationsBlock->getEquations()) {
        if (auto error = run(*equation)) {
          return error;
        }
      }

      for (auto& forEquation : equationsBlock->getForEquations()) {
        if (auto error = run(*forEquation)) {
          return error;
        }
      }
    }
    ...
    for (auto& algorithm : model->getAlgorithms()) {
      if (auto error = run(*algorithm)) {
        return error;
      }
    }

    if (auto error = resolveDummyReferences(*model)) {
      return error;
    }
