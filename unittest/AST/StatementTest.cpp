...
TEST(AST, ifStatement_nonEmptyBody)
{
        ...
        auto statement1 = Statement::assignmentStatement(
                        LOC,
                        Expression::reference(LOC,
makeType<BuiltInType::Integer>(), "x"), Expression::constant(LOC,
makeType<BuiltInType::Integer>(), 1));

        auto statement2 = Statement::assignmentStatement(
      LOC,
                        Expression::reference(LOC,
makeType<BuiltInType::Integer>(), "y"), Expression::constant(LOC,
makeType<BuiltInType::Integer>(), 2));
...
{
        auto condition = Expression::constant(LOC,
makeType<BuiltInType::Boolean>(), true);

        auto induction = Induction::build(
      LOC, "i",
                        Expression::constant(LOC,
makeType<BuiltInType::Integer>(), 1), Expression::constant(LOC,
makeType<BuiltInType::Integer>(), 2), Expression::constant(LOC,
makeType<BuiltInType::Integer>(), 1));

        auto statement = Statement::forStatement(LOC, std::move(induction),
llvm::None);
...
{
        auto condition = Expression::constant(LOC,
makeType<BuiltInType::Boolean>(), true); auto statement =
Statement::whileStatement(LOC, std::move(condition), llvm::None);
