...
TEST(ScheduledEquation, inductionVariables)
{
  ...
  auto equationOp = createEquation(builder, model, equationRanges, [&](mlir::OpBuilder& nested, mlir::ValueRange indices) {
    mlir::Value loadX = builder.create<LoadOp>(loc, model.getEquationsRegion().getArgument(0), mlir::ValueRange({ indices[0], indices[1] }));
    mlir::Value loadY = builder.create<LoadOp>(loc, model.getEquationsRegion().getArgument(1), mlir::ValueRange({ indices[0], indices[1] }));
