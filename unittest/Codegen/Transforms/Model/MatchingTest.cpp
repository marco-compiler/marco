...
TEST(MatchedEquation, inductionVariables)
{
  ...
  auto equationOp = createEquation(builder, model, equationRanges, [&](mlir::OpBuilder& nested, mlir::ValueRange indices) {
    mlir::Value loadX = nested.create<LoadOp>(loc, model.bodyRegion().getArgument(0), mlir::ValueRange({ indices[0], indices[1] }));
    mlir::Value loadY = nested.create<LoadOp>(loc, model.bodyRegion().getArgument(1), mlir::ValueRange({ indices[0], indices[1] }));
