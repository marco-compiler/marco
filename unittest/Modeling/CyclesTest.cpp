...
struct VariableTraits<Variable *> {
  ...
  static Id getId(Variable *const *variable) { return (*variable)->name(); }
};
} // namespace marco::modeling::dependency
...
struct EquationTraits<Equation *> {
  ...
  static Id getId(Equation *const *equation) { return (*equation)->name(); }
  ...
  getReads(Equation *const *equation) {
...
TEST(Cycles, selfDependency) {
 ...
 Equation::Access eq1w(&x, AccessFunction(DimensionAccess::relative(0, 1)),
"eq1w"); EXPECT_CALL(eq1, write()).WillRepeatedly(Return(eq1w));

 std::vector<Equation::Access> eq1r = {
     Equation::Access(&x, AccessFunction(DimensionAccess::relative(0, -1)),
"eq1r1"), Equation::Access(&x, AccessFunction(DimensionAccess::relative(0, 3)),
"eq1r2")
 };
...
"eq1w"); EXPECT_CALL(eq1, write()).WillRepeatedly(Return(eq1w));
 ...
 std::vector<Equation::Access> eq1r = {
     Equation::Access(&y, AccessFunction(DimensionAccess::relative(0, 9)),
"eq1r1")
 };
 ...
 Equation::Access eq2w(&y, AccessFunction(DimensionAccess::relative(0, 3)),
"eq2w"); EXPECT_CALL(eq2, write()).WillRepeatedly(Return(eq2w));

 std::vector<Equation::Access> eq2r = {
     Equation::Access(&x, AccessFunction(DimensionAccess::relative(0, -7)),
"eq2r1")
 };
...
"eq2w"); EXPECT_CALL(eq2, write()).WillRepeatedly(Return(eq2w));
 ...
 std::vector<Equation::Access> eq2r = {
     Equation::Access(&z, AccessFunction(DimensionAccess::relative(0, -4)),
"eq2r1")
 };
 ...
 Equation::Access eq3w(&z, AccessFunction(DimensionAccess::relative(0, 1)),
"eq3w"); EXPECT_CALL(eq3, write()).WillRepeatedly(Return(eq3w));

 std::vector<Equation::Access> eq3r = {
     Equation::Access(&x, AccessFunction(DimensionAccess::relative(0, -2)),
"eq3r1")
 };
...
"eq1w"); EXPECT_CALL(eq1, write()).WillRepeatedly(Return(eq1w));
 ...
 std::vector<Equation::Access> eq1r = {
     Equation::Access(&y, AccessFunction(DimensionAccess::relative(0, 9)),
"eq1r1"), Equation::Access(&z, AccessFunction(DimensionAccess::relative(0, 1)),
"eq1r2")
 };
...
"eq2r1")
 ...
 Equation::Access eq3w(&z, AccessFunction(DimensionAccess::relative(0, -4)),
"eq3w"); EXPECT_CALL(eq3, write()).WillRepeatedly(Return(eq3w));

 std::vector<Equation::Access> eq3r = {
     Equation::Access(&x, AccessFunction(DimensionAccess::relative(0, -6)),
"eq3r1")
 };
