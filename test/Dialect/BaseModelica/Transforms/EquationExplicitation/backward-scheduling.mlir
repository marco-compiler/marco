...
// CHECK-NEXT:          bmodelica.schedule_block writtenVariables = [<@x, {[0,8]}>], readVariables = [<@x, {[1,9]}>] {
// CHECK-NEXT:              bmodelica.equation_call @[[eq:.*]] {[0,8]}
...
// CHECK:           %[[i0_offset:.*]] = arith.subi %[[i0]], %[[lb]]
// CHECK:           %[[i0_remapped:.*]] = arith.subi %[[ub]], %[[i0_offset]]
...
bmodelica.model @Test {
    ...
    bmodelica.schedule @schedule {
        bmodelica.dynamic {
            bmodelica.scc {
                bmodelica.equation_instance %t0, indices = {[0,8]}, match = <@x, {[0,8]}>, schedule = [backward]
            }
        }
    }
}
