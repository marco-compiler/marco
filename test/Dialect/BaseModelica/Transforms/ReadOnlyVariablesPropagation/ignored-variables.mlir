// RUN: marco-opt %s --split-input-file --pass-pipeline="builtin.module(propagate-read-only-variables{model-name=Test ignored-variables="x,y"})" | FileCheck %s
...
bmodelica.model @Test {
    ...
    // CHECK-DAG:       %[[xValue:.*]] = bmodelica.variable.get @x
    // CHECK-DAG:       %[[yValue:.*]] = bmodelica.variable.get @y
    // CHECK-DAG:       %[[lhsValue:.*]] = bmodelica.add %[[xValue]], %[[yValue]]
    // CHECK-DAG:       %[[rhsValue:.*]] = bmodelica.variable.get @z
