...
bmodelica.model @Test {
    ...
    %t0 = bmodelica.equation_template inductions = [%i0, %i1] attributes {id = "t0"} {
        ...
        %5 = bmodelica.sub %i1, %3 : (index, index) -> index
    ...
    bmodelica.schedule @schedule {
        bmodelica.dynamic {
            bmodelica.scc {
                bmodelica.matched_equation_instance %t0 {indices = #modeling<multidim_range [1,9][1,9]>, path = #bmodelica<equation_path [L, 0]>}
            }
            bmodelica.scc {
                bmodelica.matched_equation_instance %t1 {indices = #modeling<multidim_range [0,9]>, path = #bmodelica<equation_path [L, 0]>}
            }
            bmodelica.scc {
                bmodelica.matched_equation_instance %t2 {indices = #modeling<multidim_range [1,9]>, path = #bmodelica<equation_path [L, 0]>}
            }
            ...
            // CHECK-SAME:      {
            // CHECK-SAME:          indices = #modeling<multidim_range [1,9][1,9]>
            // CHECK-SAME:          iteration_directions = [#bmodelica<equation_schedule_direction forward_1pos>, #bmodelica<equation_schedule_direction forward_1pos>]
            // CHECK-SAME:      }
            // CHECK-NEXT:  }
        }
    }
