...
int main(int argc, char* argv[])
{
  ...
  // Parse the input files
	...
	loweringOptions.openmp = openmp || idaThreads != 0;
...
mlir::LogicalResult lower(mlir::ModuleOp& module, ModelicaLoweringOptions options)
{
  ...
  passManager.addPass(codegen::createAutomaticDifferentiationPass(options.solveModelOptions));
  ...
  if (options.solveModelOptions.solver == codegen::CleverDAE)
